...
s32 main(s32 argc, char **argv, char **environ) {
    ...
    for (u32 i = 1; i < argc; i++) {
        array_push(&ctx.allocator, args, from_c_string(argv[i]));
    }
    ...
    hm_put(&ctx.allocator, visited, key, empty);

    Module module = {0};
    StringBuilder *unit = NULL;
    sb_push_str(&ctx.allocator, unit, S("// GENERATED BY MOB\n// ================\n\n"));

    for (u32 i = skip_first_arg; i < array_len(args); i++) {
        create_module_from_file(&ctx.allocator, args[i], &module);
    }

    append_strings_with_nl(&ctx.allocator, module.incs, &unit);
    append_c_typedefs_with_nl(&ctx.allocator, &module, &unit);
    append_c_macros_with_nl(&ctx.allocator, module.defines, &unit);
    ...
    for (u32 i = 0; i < array_len(module.typed); i++) {
        ...
        hm_put(&ctx.allocator, is_alias, kv.key, empty);
    }
    ...
    for (u32 i = 0; i < array_len(module.structs); i++) {
        ...
        // NOTE: cycle detected
        if (!resolve_type(&ctx.allocator, is_alias, &visited, &module, &unit, i)) {
    ...
    sb_push_char(&ctx.allocator, unit, '\n');
    append_strings_with_nl(&ctx.allocator, module.funcs, &unit);
    create_context(&ctx.allocator, &unit);

    for (u32 i = skip_first_arg; i < array_len(args); i++) {
        sb_push_str(&ctx.allocator, unit, S("#include \""));
        sb_push_str(&ctx.allocator, unit, args[i]);
        sb_push_str(&ctx.allocator, unit, S("\"\n"));
    }
    sb_push_char(&ctx.allocator, unit, '\n');
